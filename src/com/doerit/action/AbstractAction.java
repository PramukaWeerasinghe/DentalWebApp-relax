...
import com.doerit.util.SessionCreateUserAccount;
...
////session for creating user accounts
	
	public void addSessionCreateUserAccount(SessionCreateUserAccount sessionCreateUserAccount) throws SessionNotExist {
		if (session != null) {
			
			session.put(SessionKey.SESSION_CREATE_USER_ACCOUNT, sessionCreateUserAccount);
	...
	public SessionCreateUserAccount getSessionCreateUserAccount() throws SessionNotExist {
		if (session != null) {
			if (session.containsKey(SessionKey.SESSION_CREATE_USER_ACCOUNT)) {
				return (SessionCreateUserAccount) session.get(SessionKey.SESSION_CREATE_USER_ACCOUNT);
			} else {
				throw new SessionNotExist(SESSION_NOT_EXIST);
			}
		} else {
...
///end -session for creating user accounts	
	...
	public boolean isDoctor(){
		...
		try {
			...
			if (su.getRole().equalsIgnoreCase(UserRoleType.DOCTOR.getDbId())) {
				return true;
			}
		} catch (Exception e) {
			return false;
	...
	public boolean isStudent(){
		...
		try {
			...
			if (su.getRole().equalsIgnoreCase(UserRoleType.STUDENT.getDbId())) {
				return true;
			}
		} catch (Exception e) {
			return false;
	...
	public boolean isDepartment(){
		...
		try {
			...
			if (su.getRole().equalsIgnoreCase(UserRoleType.DEPARTMENT.getDbId())) {
				return true;
			}
		} catch (Exception e) {
			return false;
	...
	public boolean isReception(){
		...
		try {
			...
			if (su.getRole().equalsIgnoreCase(UserRoleType.RECEPTION.getDbId())) {
				return true;
			}
		} catch (Exception e) {
			return false;
	...
	public boolean isNurse() {
		...
		try {
			...
			if (su.getRole().equalsIgnoreCase(UserRoleType.DENTAL_NURSE.getDbId())
					|| su.getRole().equalsIgnoreCase(UserRoleType.HEALTH_MINISTRY_NURSE.getDbId())) {
				return true;
			}
	...
	public boolean authorizeVisits(){
		...
		if (isAdmin() || isNurse() || isStudent() || isDoctor() || isReception() || isDepartment()) {
			return true;
		}
		return false;
	}
